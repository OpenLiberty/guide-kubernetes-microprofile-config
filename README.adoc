...
The name resource class should look like the following when you're done.
...
Update `ping` to make the username and password members configurable. These values are used to authenticate with `name` so they are currently hard coded to `bob` and `bobpwd` respectively. Now, they will take the values of the environment variables `USERNAME` and `PASSWORD`.

Update the ping resource class in the `ping/src/main/java/io/openliberty/guides/ping/PingResource.java` file.

Add the `@Inject` and `@ConfigProperty` annotations, and remove the hard coded values.
[source, java, indent=0]
----
include::finish/ping/src/main/java/io/openliberty/guides/ping/PingResource.java[tags=credentials]
----

The ping resource class should look like the following when you're done.
[source, java, indent=0, role="no_copy"]
----
...
Replace the contents of `kubernetes.yaml` with the following:
