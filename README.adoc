...
The two microservices you will deploy are called `name` and `ping`. The `name` microservice displays a brief greeting and the name of the container that it runs in. The `ping` microservice pings the {kube} Service that encapsulates the pod running the `name` microservice. This demonstrates how communication can be established between pods inside a cluster. To build these applications, navigate to the `start` directory and run the following command.
...
Update the `name` service to have a configurable greeting message. At the moment, it is hardcoded to be `Hello!`.

Replace the `NameResource` class in the `name/src/main/java/io/openliberty/guides/name/NameResource.java` file with the following:
[source, java, indent=0]
----
include::finish/name/src/main/java/io/openliberty/guides/name/NameResource.java[tags=**;!copyright;]
----

The first change is to use MicroProfile Config and CDI to inject the value of an environment variable called `GREETING` into the `greeting` member of the name resource class.
...
The second change is to use `greeting` to create a response with a custom greeting message in the `getContainerName` method.
[source, java, indent=0, role="no_copy"]
----
...
Update the `ping` service to make the user name and password members configurable. These values are used to authenticate with the `name` service and they are currently hardcoded to `bob` and `bobpwd`. Now, they will take the values of the environment variables `USERNAME` and `PASSWORD`.

Replace the `PingResource` class in the `ping/src/main/java/io/openliberty/guides/ping/PingResource.java` file with the following:
[source, java, indent=0]
----
include::finish/ping/src/main/java/io/openliberty/guides/ping/PingResource.java[tags=**;!copyright;]
----

The changes introduced here use MicroProfile Config and CDI to inject the value of the environment variables `USERNAME` and `PASSWORD` into the ping resource class. Then the hardcoded values are removed from those same properties.
[source, java, indent=0]
----
