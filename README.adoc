...
Some basic knowledge of {kube} is assumed for this guide, and you should have a {kube} cluster available for your use. All the prerequesite knowledge is covered in the https://openliberty.io/guides/kubernetes.html[Deploying microservices to Kubernetes] guide. If you need a {kube} cluster to work with, we recommend using Docker Desktop or Minikube. Alternatively, you can provision a cluster from a cloud provider of your choice or use an existing cluster. If you choose to go with one of the local single-node clusters, you can find information on setting up Docker Desktop or Minikube at these URLs:
