...
Next, you will update your {kube} deployments to mount the ConfigMap and Secret that you created previously as volumes in the pod definition.
...
Using the [hotspot=volumes file=0]`volumes` and [hotspot=volumeMounts file=0]`volumeMounts` fields, you can populate information from various sources into different volumes in the containers. These sources include a ConfigMap, a Secret, and information about the cluster.

In this example, the `sys-app-root` ConfigMap and `sys-app-credentials` Secret are stored as the [hotspot=context-root file=0]`context-root` and [hotspot=system-app-credentials file=0]`system-app-credentials` volumes in the [hotspot=volumes file=0]`volumes` sections of [hotspot=system-container file=0]`system-container` and [hotspot=inventory-container file=0]`inventory-container`. The volumes are then mounted in the [hotspot=volumeMounts file=0]`volumeMounts` section.

Using the [hotspot=mountPath1 hotspot=mountPath2 file=0]`mountPath` field, you can specify the path to where a volume will be mapped to in the file system. The name of the file will be taken from the variable name, and the contents of the file will be the value. 

In this case, the deployment definition will result in the files `/sys_app_root/contextRoot`, `/sys_app_credentials/username`, and `/sys_app_credentials/password` being created under the `/config/variables` folder when the pod is created. Liberty will create variables from each file, where you can access by using normal Liberty variable syntax, `${sys_app_root/contextRoot}`, `${sys_app_credentials/username}` and `${sys_app_credentials/password}`. 

== Modifying system microservice

The `system` service is hardcoded to use a single forward slash as the context root and `bob` and `bobpwd` as the credentials of the basic user registry. You'll make the value of the `contextRoot` attribute configurable by implementing it as a variable. Furthermore, you'll configure the value of the context root and credentials by using the ConfigMap and Secret you created previously.
...
The `contextRoot` attribute in the [hotspot=webApplication file=0]`webApplication` element now gets its value from the [hotspot=context.root file=0]`context.root` variable. 

Set the value of the [hotspot=context.root file=0]`context.root` variable to `${system_app_root/contextRoot}`. Similarly, update the values of the [hotspot=username file=0]`system.app.username` variable to `${system_app_credentials/username}` and the [hotspot=password file=0]`system.app.password` variable to `${system_app_credentials/password}`. 

== Modifying inventory microservice

The `inventory` service is hardcoded to use a single forward slash as the context root to access the `system` service, and `bob` and `bobpwd` as the credentials to authenticate against the `system` service. You'll make these credentials configurable by implementing them as configuration variables.
...
Define the [hotspot=username file=0]`sys.app.username` and [hotspot=password file=0]`sys.app.password` variables with the values from the `sys-app-credentials` Secret. Similarly, define the [hotspot=context.root file=0]`context.root` variable with the value from the `sys-app-root` ConfigMap.
...
The changes introduced here use MicroProfile Config and CDI to inject the value of the configuration variables [hotspot=system-app-username file=1]`sys.app.username`, [hotspot=system-app-password file=1]`sys.app.password`, and [hotspot=context.root file=1]`context.root` into the [hotspot file=1]`SystemClient` class.
