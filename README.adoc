...
The tests check that the `name` service responds with a container name and that the message in the response matches the configured message. The tests for the  `ping` verify that the service returns with a `pong` response on success and that it gracefully handles a bad response. If the credentials are misconfigured, then the `ping` test will fail, so the `ping` test indirectly verifies the credentials are correctly configured.
