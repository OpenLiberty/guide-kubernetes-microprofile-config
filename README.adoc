...
These changes use MicroProfile Config and CDI to inject the value of an environment variable called [hotspot=ConfigProperty]`APP_NAME` into the [hotspot=appName]`appName` member of the `SystemResource` class.
...
The changes introduced here use MicroProfile Config and CDI to inject the value of the environment variables [hotspot=system-app-username]`SYSTEM_APP_USERNAME` and [hotspot=system-app-password]`SYSTEM_APP_PASSWORD` into the `SystemClient` class.
...
[role='command']
```
mvn failsafe:integration-test -Ddockerfile.skip=true -Dsystem.appName=my-system
```
--
