...
= Configure microservices with Kubernetes
...
== What you'll learn
The https://openliberty.io/guides/microprofile-config.html[MicroProfile Config guide] teaches you how to externalize configuration from your application, but it is not immediately clear why this is beneficial to you when developing your microservices. You will learn how to externalize your configuration, and provide the configuration values by creating environment variables. By using MicroProfile Config to handle your configuration, it provides useful annotations that you may use to inject configured values into your code whether those values come from environment variables or elsewhere.

Furthermore, you will learn how to set these environment variables using ConfigMaps and Secrets. These are resources provided by {kube} that act as a data source for your environment variables. By creating a ConfigMap or Secret, they can be used to set environment variables for any number of containers as you see fit.
...
Under the yaml for the containers, you can see the `valueFrom` field which allows you to specify the value of an environment variable from a variety of sources such as a ConfigMap, Secret, information about the cluster, and the pod's required resources. In this example `configMapKeyRef` is used to get the value `message` from the ConfigMap `greeting-config`. Similarly, `secretKeyRef` is used to get the values `username` and `password` from the Secret `name-credentials`.
