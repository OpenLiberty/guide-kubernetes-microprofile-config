...
Update `ping` to make the user name and password members configurable. These values are used to authenticate with `name` so they are currently hardcoded to `bob` and `bobpwd`. Now, they will take the values of the environment variables `USERNAME` and `PASSWORD`.
...
After you add the configuration properties for the credentials, the ping resource class should look like the following:
[source, java, indent=0, role="no_copy"]
----
