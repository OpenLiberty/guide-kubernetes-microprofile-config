...
The first change is to use the `@Inject` and `@ConfigProperty` annotations to inject the `GREETING` environment variable for use in the name resource class:
...
The second change is to update the response to use the configured greeting in the `getContainerName` method:
[source, java, indent=0, role="no_copy"]
----
...
Update the `ping` service to make the user name and password members configurable. These values are used to authenticate with the `name` service and they are currently hardcoded to `bob` and `bobpwd`. Now, they will take the values of the environment variables `USERNAME` and `PASSWORD`. Use MicroProfile Config and CDI to inject the value of the environment variables `USERNAME` and `PASSWORD` into the ping resource class. Then remove the hardcoded values from those same properties.
