...
These changes use MicroProfile Config and CDI to inject the value of an environment variable called [hotspot=17]`GREETING` into the [hotspot=16-18]`greeting` member of the [hotspot=14-30]`NameResource` class.
...
The [hotspot=22-27]`env` sections under the [hotspot=17]`name-container` is where the `GREETING` environment variable will be set.
