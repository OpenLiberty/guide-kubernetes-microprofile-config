...
The tests check that the `name` service responds with a container name, and they check that the `ping` service returns with a `pong` response on success and that it gracefully handles a bad response.
