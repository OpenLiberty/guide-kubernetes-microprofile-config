...
Next, you will update your {kube} deployments to mount each ConfigMap and Secret that you created previously to a volume in the pod definition.
...
The `sys-app-root` ConfigMap and the `sys-app-credentials` Secret are mounted in the [hotspot=env1 hotspot=volumes file=0]`volumes` sections of [hotspot=system-container file=0]`system-container` and [hotspot=inventory-container file=0]`inventory-container`.

Using the [hotspot=volumes file=0]`volumes` field, you can mount various sources into different volumes. These sources include a ConfigMap, a Secret, and information about the cluster. Then, you can specify the [hotspot=mountPath1 hotspot=mountPath2 file=0]`mountPath` for each volume mounted to where it will be mapped to in the file system under the [hotspot=volumeMounts file=0]`volumeMounts` field. Files will be read from the locations defined and the name of the file will be taken from the variable name, and the contents of the file will be the value. In this example the deployment definition will result in the files `/config/variables/system_app_root/contextRoot`, `/config/variables/system_app_credentials/username`, and `/config/variables/system_app_credentials/password` being created when the pod is created. Liberty will create variables from each file, where you can access using normal Liberty variable syntax, `${system_app_root/contextRoot}`, `${system_app_credentials/username}` and `${system_app_credentials/password}`. 

== Modifying system microservice

The `system` service is hardcoded to use a single forward slash as the context root and `bob` and `bobpwd` as the credentials of an user registry to authenticate against the microservice.

The context root is set in the [hotspot=webApplication file=0]`webApplication`
element, where the `contextRoot` attribute is specified as `"/"`. You'll make the value of the `contextRoot` attribute configurable by implementing it as a variable with value from the `sys-app-root` ConfigMap you created previously.

You'll also update the value of the [hotspot=credentials file=0]`credentials` to take from the `sys-app-credentials` Secret.
...
Update the [hotspot=context.root file=0]`context.root` variable to get its value from the `sys-app-root` ConfigMap using Liberty variable syntax, `${system_app_root/contextRoot}`. Similarly, update the values of the [hotspot=username file=0]`system.app.username` to `${system_app_credentials/username}` and [hotspot=password file=0]`system.app.password` to `${system_app_credentials/password}`. 

== Modifying inventory microservice

The `inventory` service is hardcoded to use `bob` and `bobpwd` as the credentials to authenticate against the `system` service. You'll make these credentials configurable by implementing them as configuration variables. You'll also configure the `sys-app-root` ConfigMap as a variable to relect the new context root of the `system` service.
