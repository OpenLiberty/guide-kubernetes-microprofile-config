...
MicroProfile Config provides useful annotations that you can use to inject configured values into your code. These values can come from any configuration sources, such as environment variables. To learn more about MicroProfile Config, read the https://openliberty.io/guides/microprofile-config.html[Configuring microservices^] guide.
...
The [hotspot=22-27]`env` section under the [hotspot=system-container file=0]`system-container` is where the [hotspot=23-27]`CONTEXT_ROOT` environment variable is set.
The [hotspot=50-60]`env` section under the [hotspot=45-60]`inventory-container` is where the [hotspot=51-55]`SYSTEM_APP_USERNAME` and [hotspot=56-60]`SYSTEM_APP_PASSWORD` environment variables are set.

The [hotspot=24-27 hotspot=52-55 hotspot=57-60]`valueFrom` field allows you to specify the value of an environment variable from various sources. These sources include a ConfigMap, a Secret, and information about the cluster. In this example [hotspot=25-27]`configMapKeyRef` gets the value [hotspot=27]`contextRoot` from the ConfigMap [hotspot=26]`sys-app-root`. Similarly, [hotspot=53-55 hotspot=58-60]`secretKeyRef` gets the values [hotspot=55]`username` and [hotspot=60]`password` from the Secret [hotspot=54 hotspot=59]`sys-app-credentials`.
...
Your application will now be available at the `http://[hostname]:31000/dev/system/properties`. Alternatively, you can run the following command:
...
[role=command]
```
curl http://localhost:31000/dev/system/properties -u bob:bobpwd
```
--
...
[role=command]
```
curl http://$(minikube ip):31000/dev/system/properties -u bob:bobpwd
```
--
...
The tests check that the `system` service is available at the correct context root. The tests for `inventory` verify that the service may communicate with `system` using the configured credentials. If the credentials are misconfigured, then the `inventory` test will fail, so the `inventory` test indirectly verifies the credentials are correctly configured.
