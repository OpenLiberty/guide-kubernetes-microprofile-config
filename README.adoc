...
Set the value of variable [hotspot=context.root file=0]`context.root` to `${system_app_root/contextRoot}`. Similarly, update the values of [hotspot=username file=0]`system.app.username` to `${system_app_credentials/username}` and [hotspot=password file=0]`system.app.password` to `${system_app_credentials/password}`. 
