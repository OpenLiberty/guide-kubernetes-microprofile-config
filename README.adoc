...
== View and Update ConfigMaps and Secrets

Kubernetes has the `describe` command to show details about any ressources. We can use this to display ConfigMaps and Secrets by running the following:
[role=command]
```
kubectl describe configmaps sys-app-name                                                                                                                                          
```

The output should be something similar to the following. Notice the key `name` under Data has the value `my-system`:
[source, role="no_copy"]
....
Name:         sys-app-name
Namespace:    default
...
Similarly to view the `Secret`, use the following command:
[role=command]
```
kubectl describe secrets sys-app-credential
```

The output should be similar to the following. Notice that the values for `password` and `username` are hidden.
[source, role="no_copy"]
----
...
You can quickly modify ConfigMaps by running the following command. This sets the key `name` to the value `new-name`. Note that this creates a new ConfigMaps and overwrites the old one. 
[role=command]
```
kubectl create configmap sys-app-name --from-literal name=new-name --dry-run=client --save-config=true -o yaml | kubectl replace -f -  
```
...
[source, role="no_copy"]
----
configmap/sys-app-name replaced
----

Use the `describe` command to see that the `value` for the key `name` changed.
You may redeploy the `system` service to observe the changes.

[role=command]
```
kubectl rollout restart deployment system-deployment 
```
...
You will see that the app name has changed from `my-system` to `new-name`.
...
Let's change the app name again and redeploy to prepare for testing. 

[role=command]
```
kubectl create configmap sys-app-name --from-literal name=my-system --dry-run=client --save-config=true -o yaml | kubectl replace -f -  
kubectl rollout restart deployment system-deployment 
```
