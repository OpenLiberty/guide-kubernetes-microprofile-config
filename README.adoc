...
These changes use MicroProfile Config and CDI to inject the value of an environment variable called `GREETING` into the `greeting` member of the name resource class.
...
Another change adds a new `greeting` member, it is used to create a response with a custom greeting message in the `getContainerName` method.
[source, java, indent=0, role="no_copy"]
----
