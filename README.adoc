...
:page-seo-title: Kubernetes and MicroProfile configuration tutorial
:page-seo-description: How to externalize configuration and use Kubernetes ConfigMaps and Secrets to configure your microservices
...
The tests check that the `name` service responds with a container name, and they check that the `ping` service returns a `pong` response on success and that it gracefully handles a bad response.
