...
:page-related-guides: ['kubernetes', 'microprofile-config']
...
The two microservices you will deploy are called `name` and `ping`. The `name` microservice simply displays a brief greeting and the name of the container that it runs in. The `ping` microservice simply pings the {kube} Service that encapsulates the pod running the `name` microservice. This demonstrates how communication can be established between pods inside a cluster. To build these applications, navigate to the `start` directory and run the following command.
...
Update the `PingResource` class in the `ping/src/main/java/io/openliberty/guides/ping/PingResource.java` file.

Add the `@Inject` and `@ConfigProperty` annotations, and remove the hardcoded values.
[source, java, indent=0]
----
include::finish/ping/src/main/java/io/openliberty/guides/ping/PingResource.java[tags=credentials]
----

After you add the configuration properties for the credentials, the `PingResource` class should look like the following:
[source, java, indent=0, role="no_copy"]
----
